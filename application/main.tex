...
\midrule
PI \& Main supervisor & Assoc. Prof. Giuseppe Abbiati & Department of Civil and Architectural Engineering, Aarhus University \\
...
Co-PI \& Co-supervisor & Prof. Ueli Angst & Department of Civil, Environmental and Geometic Engineering, ETH Zurich \\
\bottomrule
\end{tabular}
...
Deterioration of bridge infrastructures is a critical issue in the \emph{west}, where such assets, in large part, already exceed their nominal life. Europe has a very dense network of transportation infrastructures. The prioritization of what to repair and what to demolish is a critical challenge. The catastrophic collapse of the Morandi Bridge (Genova, Italy, 2018) indicates that bridges in poor condition may still be operated. The situation in \emph{global south} is characterized by younger infrastructures which, however, due to underfunded maintenance programs are likely to experience fast deterioration. Vibration-based SHM has been implemented extensively in the last 20 years along with the large diffusion of cost-effective sensing technology, broadband internet access, and connectivity through IoT~\cite{figueiredo_three_2022}. More recently, UASs have become affordable and easy to operate, facilitating the capture of high-resolution image data from hard-to-access structural elements~\cite{panigati_drone-based_2025}. However, engineers must still combine these heterogeneous sources to determine whether an observed condition is significant, whether additional inspection is needed, and which intervention to prioritize~\cite{hoskere_unified_2025}. The overwhelming amount of data, along with the shortage of qualified personnel, renders this a challenging task. \par
%
...
\begin{enumerate}[label=Q\arabic*]
    ...
    \item how semantic representation shall be integrated with AI to maximize likelihood of logical consistency in reasoning? \label{question:integration}
...
\end{enumerate}
%
AU-CAE will tackle \ref{question:sematic} and \ref{question:integration} while ETHZ will tackle \ref{question:interactive} and \ref{question:immersive}. The key-enabling technologies involved in this project are 1) semantic representations, 2) conversational agentic AI 3) and XR providing the immersive layer. \par
%
...
%
The overall scientific contribution will be a new paradigm for an evidence-based decision-support system with immersive human-interaction facilitated by AI and AR. Three journal publications in top-tier peer-reviewed journals\footnote{For example, Automation in Construction (Elsevier), Structural Control and Health Monitoring (Wiley)} in the field are expected. %supported by a semantic SHM representation, a multimodal graph-grounded reasoning architecture, an AR-enabled human-interaction framework, and experimental evidence on decision performance and data sufficiency. 
...
\textbf{RF7 -- Extending the life of the built environment (primary).} \projectname addresses the RF7 mission of ``decision support that integrates structural health monitoring information'' \cite[p.~132]{cebe_research_roadmap_2026}. Within the RF7 activities on assessing the capacity and residual life of structures, it builds on the AI/robot-assisted operation of SHM systems \cite[p.~148]{cebe_research_roadmap_2026} by adding the layer that turns their observations into condition hypotheses linked to deterioration mechanisms such as rebar corrosion. Within the RF7 decision-support frameworks, it contributes to practical guidance for risk-informed decisions on repair, retrofitting, and transformation \cite[p.~145]{cebe_research_roadmap_2026} by quantifying how engineers rely on AI-supported assessments. It thereby targets two barriers identified for RF7: limited confidence in decisions based on scarce monitoring data, and the adoption of new digital tools by practitioners \cite[p.~141]{cebe_research_roadmap_2026}.%, and provides evidence for the impact lever ``decision makers favour adaptation over demolition''. The key contribution is a human--AI decision process validated in terms of decision quality.%, not algorithmic accuracy alone.
...
\textbf{RF4 -- Digitalisation and automation.} The semantic DT and KG of WP1 extend the semantic modelling, reality capture and interoperability of WP4.4--4.6 \cite[pp.~79--80]{cebe_research_roadmap_2026} from buildings to in-service bridge infrastructure. The project will contribute to the RF4/RF7 knowledge exchange on DT engineering and agentic AI \cite[p.~146]{cebe_research_roadmap_2026}.
...
\textbf{RF5 -- Climate resilient and adaptive infrastructure.} Water ingress is driven by precipitation and exposure. The KG will represent environmental exposure as context for interpreting moisture and corrosion symptoms, interfacing with RF5 work on climate and infrastructure data (WP5.1), and it will supply asset-level evidence to risk-based adaptation decisions (WP5.5) \cite[p.~102]{cebe_research_roadmap_2026}.
...
\textbf{RF1 -- Assessing and measuring environmental sustainability.} The environmental consequences of case-study decisions will be screened with the RF1 sustainability framework (see Section~4) \cite[p.~24]{cebe_research_roadmap_main_2026}.
...
\textbf{SDG perspective:} Bridge infrastructures connect settlements and guaranty the delivery of goods and services. The rise in temperature and increase in rainfall negatively impact the corrosion mechanisms affecting common construction materials like concrete and steel. Therefore \projectname SDG 11 (Sustainable Cities and Communities) and SDG 13 (Climate Action).
%Evidence-grounded decision support can help prioritize structural components requiring additional investigation or intervention while reducing unnecessary inspections and avoiding excessive permanent instrumentation while ensuring that critical infrastructures remain viable.
% Therefore, \projectname adddress 

%\textbf{Climate Action (13):}  Underfunded inspection programs are likely to fail in tracking critical deterioration scenarions. \projectname aim to automate the process so that it can be deployed in large scale with limited qualified personnel. 

%Better identification of water ingress, drainage failures, moisture exposure, and associated deterioration may enable localized preventive interventions before damage develops into more material-intensive rehabilitation or premature replacement.

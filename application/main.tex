...
\newcommand{\projectname}{\textbf{PROJECT\textunderscore NAME }}
\newcommand{\asset}{\textbf{BRIDGE\textunderscore NAME }}
...
Critical infrastructure is increasingly affected by aging, climate exposure, water ingress, and limited maintenance resources, while bridge inspection generates growing volumes of heterogeneous information from Uncrewed Aerial Systems (UAS), sensors, inspection reports, and 3D models. The emerging challenge is to support engineers what the combined evidence means, how certain that interpretation is, and what steps should be taken next, in collaborative workflows that combine AI with engineering expertise.

This PhD will develop and validate a human-centered situational-awareness framework for bridge Structural Health Monitoring (SHM). The framework will integrate UAS imagery, sensor measurements, structural information, inspection histories, environmental observations, and engineering knowledge through a DT (DT) and a knowledge graph. Knowledge-grounded AI based on LLMs and VLMs will support interpretation of this evidence by identifying relevant relationships, plausible deterioration mechanisms, uncertainty, and information gaps. 
...
Bridge inspection and Structural Health Monitoring (SHM) are undergoing rapid digital transformation. Unmanned Aerial Systems (UAS) can capture high-resolution information from hard-to-access structural elements, sensors can continuously monitor structural and environmental variables, CV can automatically classify anomalies. DTs (DTs) can provide virtual replicas of real-world assets by integrating information from multiple sources.
...
AI models may provide an opportunity for engineers to rapidly access and interpret a variety of data sources quickly to support them in their decision-making process. However, effective human-AI collaboration are only beginning to emerge and remain particularly underexplored for inspection and SHM workflows. 
...
\begin{enumerate}[label=F\arabic*]
    ...
    \item to create a human interaction layer based on AR and conversational agents through which the DT operators evaluate the evolving asset situation in an immersive environment with reduced cognitive load.\label{focus:immersive}
\end{enumerate}
%
...
\begin{enumerate}[label=Q\arabic*]
    ...
    \item how the DT operator shall interact with the immersive environment to improve situation awareness while reducing cognitive load? \label{question:immersive}
\end{enumerate}
%
AU-CAE tackle \ref{question:sematic} while ETHZ \ref{question:immersive}. The key-enabling technologies involved in this project are 1) KGs, used to cast semantic representations, 2) LLMs, used to implement agentic AI and 3) AR, used to create the human interaction layer.
%
...
Research in bridge SHM has achieved important advances in automated inspection, sensing, CV, and DTs.

UAS and photogrammetry improve inspection access and provide detailed geometric and visual information, while machine-learning approaches can identify cracks, spalling, corrosion, moisture traces, and other visible defects. 
...
The project will develop and experimentally validate a human-centered framework for bridge Structural Health Monitoring (SHM) in which a semantic DT (DT), multimodal Artificial Intelligence (AI), and Augmented Reality (AR) work together to transform heterogeneous inspection data into traceable engineering knowledge and decision support. 
%
...
\textbf{WP1 Asset knowledge representation}
%
The main scientific contribution of WP1 will be a semantic model that links all digital assets associated with the \asset.
%
\begin{itemize}
    \item \textbf{T1.1 Digital assets:} Technical drawings, historical data, maintenance reports associated with the \asset will be gathered and dissected. Based on this input, a BIM and a FEM model of the \asset will be produced. The FEM will be used to describe the mechanical response of the brdige and to generate synthetic response data whereas the BIM model will describe its geometry. Both available experimental response data and synthetic response data will be organize into databases (e.g., \texttt{InfluxDB} for time-series data and \texttt{MongoDB} for text data).\par
    \item \textbf{T1.2 Semantic description:} A semantic model will be develop to describe the condition of the \asset including history of inspection data and maintenance reports. The BIM model developed in T1.1 will be used to derive a semantic layer for the \asset geometry. Additional semantic layers will be included for \emph{deterioration mechanisms} (e.g., cracking, corrosion), \emph{measurements} (e.g., UAS imagery, photogrammetric models), \emph{inspection reports}, \emph{maintenance plans}. The DT semantic description will be implemented as KGs.\par
\end{itemize}
%
\textbf{WP2 Multimodal and Graph-Grounded Artificial Intelligence}
%
The main scientific contribution of WP2 will be a multimodal, graph-grounded AI architecture for traceable infrastructure reasoning that moves beyond defect recognition toward evidence-based interpretation of bridge condition.\par
%
\begin{itemize}
    \item \textbf{T2.1 Automated annotation of deterioration:} WP2 will investigate how AI can transform heterogeneous observations into evidence-grounded interpretations of bridge condition. CV and VLM technologies will identify and contextualize candidate observations from UAS imagery, photographs, inspection documentation, and other multimodal information. The objective is not to develop another universal defect detector, but to determine how automatically extracted observations can become structured evidence within the semantic DT.\par
    \item \textbf{T2.2 Automated reasoning on structure condition:} A graph-grounded LLM/VLM reasoning architecture will retrieve relevant asset-specific information from the KG before generating an engineering interpretation. For example, when moisture is detected below an expansion joint, the system may retrieve the joint's condition and inspection history, drainage configuration, recent rainfall exposure, nearby cracking or corrosion observations, bearing locations, previous repairs, and relevant engineering knowledge before proposing plausible deterioration hypotheses.\par
    \item \textbf{T3.2 Human interaction:}
\end{itemize}
%
...
%
The overall scientific contribution will be a new paradighm for evidence-based decision-support system with immersive human-interaction facilitated by AI and AR. %supported by a semantic SHM representation, a multimodal graph-grounded reasoning architecture, an AR-enabled human-interaction framework, and experimental evidence on decision performance and data sufficiency. 
Potential practical impacts include faster interpretation of inspection information, reduced effort spent searching fragmented records, better prioritization of inspection resources, and more transparent and traceable documentation of scientific evidence in support to engineering decisions.
